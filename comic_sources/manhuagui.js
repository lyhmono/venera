...
class ManHuaGui extends ComicSource {
  ...
  comic = {
    ...
    loadEp: async (comicId, epId) => {
      ...
      let document = await this.getHtml(url);
      // Site obfuscates eval as window["\x65\x76\x61\x6c"]; packed script
      // moved from index 4 to 5 after a template update. Find it by signature
      // instead of hardcoding the position.
      let scripts = document.querySelectorAll("script");
      let script = null;
      for (let s of scripts) {
        if (s.innerHTML && /function\(p,a,c,k,e,d\)/.test(s.innerHTML)) {
          script = s.innerHTML;
          break;
        }
      }
      if (script == null) {
        throw "packed script not found";
      }
